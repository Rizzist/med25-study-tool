...
export function BiochemistryCoreCard({saved,onOpen}:{saved:unknown;onOpen:(selection:ExamCollection,fresh?:boolean)=>void}) {
  const [manifest,setManifest]=useState<CoreManifest|null>(null),[error,setError]=useState(''),[scope,setScope]=useState('');
  const [retry,setRetry]=useState(0);
  useEffect(()=>{let cancelled=false;setError('');void cachedJson<CoreManifest>('/study/biochemistry/core-exam.json').then(data=>{if(!cancelled)setManifest(data);}).catch(e=>{if(!cancelled)setError(e.message);});return()=>{cancelled=true;};},[retry]);
  ...
  if(!manifest)return <p role="status" className="mcq-loading">Loading Biochemistry Core selection and repeat counts…</p>;
  const full=coreSelection(manifest),repeat=coreSelection(manifest,'repeats');
  function launch(selection:ExamCollection) {
    const previous=paperAttemptSummary(saved,'term2-biochemistry',selection);
  ...
  return <div className="biochemistry-core-card"><PastPaperCard featured badge="★ Start here" label="Core exam · selected PYQs" title="Biochemistry Core Exam" note={manifest.methodology}>
    ...
    <p>{manifest.sourceQuestionCount} is the total across {manifest.sourceCollectionCount} main source collections—not the number of repeats. One representative per detected pattern; no 100-question cap. The {manifest.optionalQuestionCount}-item reconstruction is excluded.</p>
    <div className="paper-card-actions">{launch(full)}{launch(repeat)}</div>
    <div className="core-section-picker"><label htmlFor="biochem-core-section">Or focus a review section</label><select id="biochem-core-section" value={scope} onChange={e=>setScope(e.target.value)}><option value="">Choose a section…</option>{manifest.sections.map(s=><option key={s.id} value={s.id}>{s.title} · {s.count}</option>)}</select>{scope&&launch(coreSelection(manifest,scope))}</div>
