...
type Props = {
  ...
  attemptedIds: string[];
  repairIds: string[];
  initialPanel?: "questions" | "learn" | "watch" | "recall";
};
...
export function PhysiologyPracticalHub({ onPractice, disabled, attemptedIds, repairIds, initialPanel = "questions" }: Props) {
  ...
  const questionSets = [{ id: "core", title: "Core station essentials", source: station.source, locator: "Original starter set · saved progress retained", questionIds: station.coreQuestionIds }, ...station.theorySets];
  ...
  return <div className="pp-hub">
    ...
    <div className="pp-layout">
      ...
      <section className="pp-station" aria-label={station.title}>
        ...
        <nav className="pp-panels" aria-label="Lesson view">{([['questions', '01 / Question sets'], ['learn', '02 / Understand'], ['watch', '03 / Figures & video'], ['recall', '04 / Recall & coverage']] as const).map(([id, label]) => <button key={id} aria-pressed={panel === id} className={panel === id ? 'active' : ''} onClick={() => setPanel(id)}>{label}</button>)}</nav>
        <div className="pp-panel" key={station.id + panel}>
          {panel === "questions" && <>
            ...
            <div className="pp-question-filters"><label htmlFor="pp-question-search">Search this station<input id="pp-question-search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Try dilution, interval, cuff…" /></label><label htmlFor="pp-question-filter">Show<select id="pp-question-filter" value={filter} onChange={(event) => setFilter(event.target.value as PracticalQuestionFilter)}><option value="all">All questions</option><option value="new">New theory questions</option><option value="unseen">Not attempted yet</option><option value="repair">Needs repair</option><option value="calculations">New calculation cases</option><option value="images">Image questions</option></select></label></div>
