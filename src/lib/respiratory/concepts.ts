...
import { livePracticeCatalog, livePracticeIndex } from "@/src/lib/mcq/practice-catalog";

export type RespiratorySubject = "anatomy" | "histology" | "embryology" | "physiology";
export type RespiratorySource = { title: string; edition?: string; chapter?: string; locator: string; basis: "book" | "slides" | "transcript" | "notes" | "media" };
...
export type RespiratoryConcept = {
  ...
  sources: RespiratorySource[]; scope: "book-and-slides" | "book-only" | "slide-only" | "book-and-course" | "course-only"; objectives: RespiratoryObjective[];
};
...
export type RespiratoryQuestionIndex = Record<string, { subject: RespiratorySubject; kind: string; moduleId: string; moduleIds: string[]; conceptIds: string[]; primaryConceptId: string; dedupeKey: string; prompt: string; learningObjective: string; addedForGap: boolean; difficulty?: number; knowledgeLevel?: "core" | "challenge"; expandedForDepth?: boolean }>;

export const respiratoryConcepts = livePracticeCatalog(catalog).concepts as RespiratoryConcept[];
...
export const respiratoryQuestionIndex = livePracticeIndex(index) as RespiratoryQuestionIndex;
...
export const respiratoryScopeLabels = { "book-and-slides": "Book + slides", "book-only": "Book only", "slide-only": "Slides only", "book-and-course": "Book + course transcript/notes", "course-only": "Course transcript/notes" };
