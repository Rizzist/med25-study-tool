...
export function reviewBreakdown(outcomes,course) {
  ...
  for(const item of outcomes) {
    ...
    if(!row){row={id,sectionId:section?.id,title:section?.title||(course?'Not yet matched to a review section':item.topic||'Other topics'),uncertain,correct:0,answered:0,skipped:0,ungraded:0,total:0,missedIds:[]};groups.set(id,row);}
    row.total++;
    if(!item.answered){row.skipped++;row.missedIds.push(item.questionId);continue;}
    if(item.gradable===false){row.ungraded++;continue;}
    row.answered++;if(item.correct)row.correct++;else row.missedIds.push(item.questionId);
  }
