import type { AiAnswer } from './cvs-paper-enhancements.mjs';
export type PaperQuestion = {id:string;number:string;page:number;prompt:string;options:string[];providedKey:string|null;scoringKey:string|null;keyNote:string;issues:string[];sourcePage:string;media?:string;aiAnswer?:AiAnswer;originalPrompt?:string;originalOptions?:string[];correctionRevision?:string;originPaper?:{id:string;title:string;sourceUrl:string;transcriptUrl:string}};
export type Paper = {id:string;title:string;fingerprint:string;questions:PaperQuestion[];note:string;sourceUrl:string;keyStatus:string;transcriptUrl:string;sourcePaperIds?:string[];aiRevision?:string};
export type PaperFeedbackMode = 'instant'|'deferred';
export type PaperQuestionFeedback = 'unanswered'|'correct'|'incorrect'|'ungraded';
export type PaperAttempt = {paperId:string;fingerprint:string;answers:Record<string,string>;index:number;startedAt:string;completedAt:string|null;manual:Record<string,string>;feedbackMode?:PaperFeedbackMode;sourcePaperIds?:string[];correctionRevisions?:Record<string,string>};
export type PaperCounts = {
  total:number; answered:number; correct:number; incorrect:number; unanswered:number; ungraded:number;
  manualCorrect:number; manualIncorrect:number; sourceKeyed:number; percentage:number|null;aiKeyed?:number;aiCorrect?:number;aiIncorrect?:number;
};
...
export type PaperResult = {paperId:string;fingerprint:string;completedAt:string;total:number;keyed:number;matched:number;unanswered:number;manualCorrect:number;manualGraded:number;ungraded:number;percentage:number|null;sectionStats?:PaperBreakdown;aiKeyed?:number;aiMatched?:number;aiRevision?:string;sourcePaperIds?:string[]};
