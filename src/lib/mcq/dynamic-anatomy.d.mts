...
export type AnatomySourceImage = {
  ...
  source: MCQSource; attribution?: string; labelStyle?: string;
  practicalExpansion?: boolean; galleryOnly?: boolean; studyOnly?: boolean; studyRegions?: AnatomySourceImage['regions']; practiceNote?: string; supersedesImageIds?:string[];
  anatomy3d?: { modelKey: string; structureIds: string[] };
  labelMasks?: Array<{ x: number; y: number; width: number; height: number }>;
  regions: Array<{ id: string; label: string; description: string; x: number; y: number; width: number; height: number; structureId?: string; modelKey?: string; contextStructureIds?: string[]; distractorRegionIds?: string[]; sourceNumber?: string | number }>;
};
