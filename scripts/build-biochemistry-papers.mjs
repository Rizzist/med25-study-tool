...
for(const paper of data.papers){
 ...
 for(const original of paper.questions){
  ...
  const record={...row,id,sectionId:mapping.sectionId,uncertain:mapping.uncertain,answerReview:{basis,confidence:resolution?.confidence??(row.note||mapping.uncertain?'medium':'high'),canonicalSourceId:id,auditedAt:paper.importedAt??data.importedAt,evidence:refs},explanation};
  records.push(record);
  if(!row.key)continue;
