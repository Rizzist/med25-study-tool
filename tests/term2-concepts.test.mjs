...
test("verified CVS, limbs and biochemistry concept maps remain exhaustive and internally linked", async () => {
  ...
  for (const item of expected) {
    ...
    const practical = questions.filter(q => q.tags.includes("biochemistry-practical"));
    const practicalConcepts = catalog.concepts.filter(c => c.id.startsWith("biochem2-practical-"));
    const practicalModules = catalog.modules.filter(m => m.id.startsWith("biochem2-practical-"));
    const limbScience = questions.filter(q => q.tags.includes("limb-science-audit"));
    const limbScienceConcepts = catalog.concepts.filter(c => c.id.startsWith("limb-audit-"));
    const limbScienceModules = catalog.modules.filter(m => m.id.startsWith("limb-audit-"));
    ...
    assert.equal(catalog.concepts.length, item.concepts + visualFigures + depthConcepts.length + practicalConcepts.length + limbScienceConcepts.length);
    assert.equal(catalog.modules.length, item.modules + (visualFigures ? 1 : 0) + depthModules.length + practicalModules.length + limbScienceModules.length);
    assert.equal(catalog.concepts.flatMap((concept) => concept.objectives).length, item.objectives + visualTargets + depthConcepts.flatMap(c => c.objectives).length + practicalConcepts.flatMap(c => c.objectives).length + limbScienceConcepts.flatMap(c => c.objectives).length);
    assert.equal(questions.length, item.questions + visual.length + location.length + depth.length + practical.length + limbScience.length);
    assert.equal(storedCoverage.addedQuestionCount, item.gaps + depth.length + limbScience.length);
    assert.equal(storedCoverage.imageQuestionCount, item.images + visual.length + location.length + depth.filter(q => q.media?.length).length + practical.filter(q => q.media?.length).length + limbScience.filter(q => q.media?.length).length);
